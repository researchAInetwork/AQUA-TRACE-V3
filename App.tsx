...
function App() {
  ...
  const resize = useCallback((e: MouseEvent) => {
    if (isResizing) {
      // Constraints: 320px min, 800px max
  ...
  return (
    <div className={`flex flex-col h-screen bg-slate-950 font-sans selection:bg-cyan-500/30 overflow-hidden ${isResizing ? 'cursor-col-resize select-none' : ''}`}>
      ...
      <main className="flex-1 flex flex-col lg:flex-row min-h-0 overflow-hidden relative">
        ...
        <button 
          ...
          className={`lg:hidden fixed bottom-24 right-6 z-[70] w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 border-2 ${
            isSidebarOpen ? 'bg-slate-800 border-slate-700 text-slate-400 rotate-180' : 'bg-cyan-600 border-cyan-400 text-white'
          }`}
        >
          {isSidebarOpen ? (
        ...
        <aside 
          ...
          bg-slate-950 lg:bg-slate-950
          lg:border-r border-slate-800/60 
        ...
        `}>
          ...
          {/* Professional Dual-Purpose Handle: Resize + Toggle */}
          <div
          ...
          >
            ...
            <div 
              ...
              className={`
                absolute left-2.5 w-6 h-12 bg-slate-900 border border-slate-800 
              ...
              `}>
               <svg className={`w-3.5 h-3.5 transition-colors ${isSidebarOpen ? 'text-slate-500 group-hover/handle:text-cyan-400' : 'text-cyan-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M15 19l-7-7 7-7" />
              </svg>
            </div>
          </div>

          {/* Internal Content Rail */}
          <div 
              ...
              <div className="space-y-8">
                {!preview ? (
                  <div className="space-y-6">
                    <div className="flex p-1.5 bg-slate-950/80 rounded-xl border border-slate-800/50 shrink-0">
                      <button 
                    ...
                    <div className="aspect-[4/3] w-full relative">
                      {inputMode === 'upload' ? (
                        <div 
                          ...
                          className={`h-full border-2 border-dashed rounded-2xl flex flex-col items-center justify-center cursor-pointer transition-all ${isDragging ? 'border-cyan-500 bg-cyan-500/10' : 'border-slate-800/50 hover:border-slate-600 bg-slate-900/40 shadow-inner'}`}
                          onClick={() => fileInputRef.current?.click()}
                        >
                          <div className="w-14 h-14 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center mb-4 transition-transform hover:scale-110">
                            <svg className="w-7 h-7 text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
                          </div>
                          <p className="text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">Drop Forensic Media</p>
                    ...
                    {inputMode === 'camera' && !preview && (
                       <div className="bg-cyan-950/20 border border-cyan-900/40 p-5 rounded-2xl space-y-3 animate-pulse">
                          <p className="text-[10px] font-black text-cyan-400 uppercase tracking-widest leading-none">AQUA-OPTIC Active</p>
                          <p className="text-[12px] text-slate-400 font-medium leading-relaxed">Stabilize the lens for precision sheen detection and multi-point impact triangulation.</p>
                       </div>
                    )}
                  </div>
                ) : (
                  <div className="relative group rounded-[32px] border border-slate-800/80 overflow-hidden bg-black aspect-[4/3] shadow-2xl">
                    {isVideo ? (
                ...
                <div className="space-y-8">
                  <div className="space-y-3">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.25em]">Inference Granularity</p>
                    <div className="flex p-1.5 bg-slate-950 rounded-xl border border-slate-800/50">
                      <button onClick={() => setGranularity('Standard')} className={`flex-1 py-2.5 text-[11px] font-black rounded-lg transition-all ${granularity === 'Standard' ? 'bg-cyan-600 text-white shadow-lg shadow-cyan-900/40' : 'text-slate-500 hover:text-slate-300'}`}>STANDARD</button>
                      <button onClick={() => setGranularity('Expert')} className={`flex-1 py-2.5 text-[11px] font-black rounded-lg transition-all ${granularity === 'Expert' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'text-slate-500 hover:text-slate-300'}`}>EXPERT</button>
                    </div>
                  </div>

                  <div className="space-y-3">
                    <div className="flex justify-between items-center px-1">
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.25em]">METADATA</label>
                      <span className="text-[9px] font-bold text-cyan-800 uppercase tracking-widest px-2.5 py-1 bg-cyan-950/20 rounded-lg border border-cyan-900/30">Optional</span>
                    </div>
                    <textarea value={context} onChange={(e) => setContext(e.target.value)} placeholder="Enter site conditions, odor profiles, or upstream anomalies..." className="w-full h-32 lg:h-40 bg-slate-950/80 border border-slate-800 rounded-[24px] p-5 text-[14px] text-slate-300 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/20 transition-all resize-none placeholder:text-slate-800 font-medium" />
                  </div>

                  <button 
                    onClick={handleAnalyze} 
                    disabled={!file || analysisState.status === 'analyzing'} 
                    className={`w-full py-5 rounded-[24px] font-black text-[13px] uppercase tracking-[0.4em] transition-all relative overflow-hidden group shadow-2xl ${!file || analysisState.status === 'analyzing' ? 'bg-slate-800 text-slate-600 grayscale' : 'bg-cyan-600 hover:bg-cyan-500 text-white shadow-cyan-900/40 active:scale-95'}`}
                  >
        ...
        <section className={`flex-1 flex flex-col min-h-0 bg-slate-950 relative overflow-hidden transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) ${isSidebarOpen && window.innerWidth < 1024 ? 'opacity-20 pointer-events-none blur-xl' : 'opacity-100'}`}>
              ...
              <div className="h-full flex flex-col max-w-6xl mx-auto">
                {analysisState.status === 'analyzing' ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-center space-y-12 animate-fade-in-up">
                    ...
                    <div className="space-y-6">
                      <h3 className="text-3xl sm:text-5xl font-black text-white tracking-tighter uppercase">Analyzing Multi-Modal Telemetry</h3>
                  ...
                  <div className="space-y-12 sm:space-y-24 py-10 lg:py-20">
                    <div className="space-y-8 sm:space-y-10">
                      <div className="inline-flex px-5 py-2 rounded-full bg-cyan-950/20 border border-cyan-900/50 text-[11px] font-black text-cyan-400 uppercase tracking-widest shadow-lg shadow-cyan-950/40">Sovereign Sentinel Active</div>
                      <h2 className="text-5xl sm:text-7xl lg:text-9xl font-black text-white leading-[1] tracking-tighter">Next-Gen <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-indigo-600">Water Impact Intelligence</span></h2>
                      <p className="text-slate-400 text-xl sm:text-2xl lg:text-3xl leading-relaxed max-w-4xl font-medium tracking-tight">Enterprise field screening for rapid triage and ecological risk prioritization using multimodal reasoning engines.</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-12">
                      {[
                        ...
                        <div key={i} className="group p-10 sm:p-12 bg-slate-900/30 border border-slate-800/40 rounded-[48px] space-y-8 transition-all hover:bg-slate-900/50 hover:border-slate-700 shadow-2xl">
                          <div className="w-16 h-16 bg-cyan-950/40 rounded-[20px] border border-cyan-500/20 flex items-center justify-center text-cyan-400 group-hover:scale-110 transition-transform">
                          ...
                          <div className="space-y-4">
                            <h4 className="text-white font-black text-base uppercase tracking-widest leading-none">{item.title}</h4>
                            <p className="text-base text-slate-500 leading-relaxed font-semibold">{item.text}</p>
                          </div>
                        </div>
                      ))}
                    </div>
      ...
      <footer className="h-20 shrink-0 bg-slate-950 border-t border-slate-900 px-8 flex items-center justify-between text-[11px] text-slate-500 font-mono z-50 overflow-hidden shadow-[0_-10px_50px_rgba(0,0,0,0.5)]">
        ...
        <div className="text-slate-800 font-black tracking-[0.4em] whitespace-nowrap hidden sm:block uppercase opacity-50">©2024 ENV-SYS INTELLIGENCE SYSTEM v1.2</div>
      </footer>
    </div>
  );
}
